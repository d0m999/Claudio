...
做任何视觉 / UI 决策前，**先读 [DESIGN.md](./DESIGN.md)**。字体、配色、间距、圆角、动效、五事件可视体系、美学方向都定义在那里。
...
- 品牌强调唯一 = 黏土 `#D97757`；主图形 = **C / Signal**；五事件语义色（TaskStart info 蓝 / Stop 绿 / StopFailure 琥珀 / Notification 黏土 / SubagentStop 靛）；**StopFailure 绝不用红**；真红 `#FF453A` 只给 App 自身真错误。
