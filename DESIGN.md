...
各自凭据状态；包侧栏固定显示 `n/5`。声音页默认选中第一个事件；点击事件名称、开始生成或从
缺声深链进入时，在该事件行下方固定显示 Claude Code、Codex、WorkBuddy 三行来源对照，切换事件
时更新同一处对照，不另设能力查看动作。主视图使用「支持」「仅授权请求可用」「暂不可用，
尚未接入」和「不可用，这个来源没有此类事件」；原生事件名与精确能力状态放在默认收起的「技术信息」
中。对照只投影 `HostCapabilityBinding` 的事件能力，不承诺当前一定播放；实际能否听到声音还取决于
是否选用此包、连接、静音及该事件是否已有音频。能力不限制生成。可直接向健康用户包采纳；「提示音组」
是普通用户包的界面称呼，新建空组在
首音采纳成功前不入库；任意健康包复制成功即发布，普通复制只查看副本。缺声深链对只读内置包
