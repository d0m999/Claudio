...
- **⑥ 覆盖轨 = 横排糖豆胶囊**：`present` = 实心胶囊 **≈14×7pt**（事件色）· `missing` = 空壳 + 斜杠（**形状区分，不靠调灰**）。竖排 slot 版随卡片画廊一并消解。管理窗口微型档同形状缩几何（≈9–10×5、间距 2.5–3）。覆盖 v1「面板行档 18×10 / 微型 9×9」的几何；「恒显 / `broken` 不渲染 / `CC0` 与缺 N 个双槽」等**语义层不变**（见「4-slot 覆盖轨」「包行四态」）。
...
- **包行四态 `PackCardState{complete | partial | broken}` × 正交的 `isSelected`（2026-07-15 定态模型 · /plan-design-review；2026-07-17 形制改竖排整宽行 · 用户拍板 mockup）**
  ...
  - **`broken`（目录/manifest 读不出）** —— meta 槽 = 真红 `xmark.circle.fill` + `文件丢失`（`text-2`，真红只上图标，沿用「拒绝行」语言）；**覆盖轨不渲染** —— 画不出没读出来的覆盖度，宁可空着也不用 4 个空槽谎报「读出来了、全缺」。
...
- **4-slot 覆盖轨（新组件 · 2026-07-17 起在面板包行上**恒显**；`broken` 行不渲染 —— 画不出没读出来的覆盖度）**
  ...
  **（⚠ 2026-07-17 糖果盘覆盖几何，见「现行视觉皮肤：糖果盘」⑥）**：面板行档改**糖豆胶囊 ≈14×7**（present 实心 / missing 空壳 + 斜杠）、微型档 ≈9–10×5、间距 2.5–3 —— 上文 18×10 / 9×9 为 v1 存档。**形状语义（present 实心 vs missing 空壳斜杠、恒显、`broken` 不渲染）不变，只缩几何、圆得更足。**
