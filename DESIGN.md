...
## 默认组与工作区（现行 · 2026-09-25）

本节替代下文历史 Global/Surface 声音选择、稀疏覆盖、唯一全局主音量与按声音作用域统计活动的规则；字体、主题、固定 312pt 面板宽度与单一 retained 设置窗口继续有效。下文标为 2026-09-06 的旧面板解剖及四档字号画廊只供追溯，不恢复其声音或字号模型。

- 「事件与提示音」改为「默认组／工作区」：默认组与工作区列表进入同窗口详情。详情提供完整声音包、独立音量、五事件、目录范围、适用 Surface、试听及真实适用状态。添加目录从子目录明确定位仓库根；必须选包并确认音量，事件初始全开。
...
- 删除工作区规则先显示原生确认：标明工作区名称，同名时补本机目录摘要，说明只删声音规则，项目文件与声音包保留；之后按剩余规则与默认组重新匹配。请求时捕获稳定规则 ID，取消、Escape、关闭或离页不写入；取消是安全初始动作。确认只消费捕获目标一次，写入前重新验证；失效目标不能改写新选择或默认组。成功后选中默认组、显示结果并聚焦可达位置，取消后回到删除按钮；拒写保留原值与选择，错误即使详情不可写也可见可达。发布后并发冲突须读回并说明结果不确定，不自动重试。
- 集成页声音入口使用「默认组／工作区…」（English: “Default Group & Workspaces…”），无障碍提示同义。它是普通目的页入口：保留会话中有效的手动声音作用域，首次显示默认组，不按所选 host 改选；失效作用域显示不可用和显式选择入口。显式工作区／事件深链接仍校验目标，不静默落到可写默认组。
...
- **⑤ 事件行 mono id（2026-08-25 历史作用域文案）**：当时的面板在 Surface 作用域显示宿主原生事件名，在 Global 显示 claudi0 事件 ID；这组来源级选择规则已由上方默认组／工作区合同覆盖。等宽次要文字规范及管理窗口的原生事件和 manifest 映射细节继续有效。
- **③④ 面板包列表（2026-08-25 废止）**：旧「星标 ≤4 + 覆盖轨」仅作为历史组件与管理窗口资料保留，生产面板不再挂载声音包画廊。当时的包行和「打开设置」旧窗口路由只作沿革；现行入口进入同一设置窗口的默认组／工作区详情及「声音」目的页。
...
**2026-08-02 落地结论**：两笔旧债均已关闭。`ClaudioColorHex` 与 `ContrastSuite` 已按最深渐变底 `#FBF7F1` 验证；未立项的全局静音控件正式删除，当时的主音量行只保留滑块。

## 全产品界面职责（现行 · 2026-09-12）

产品只保留两个顶层 surface，不再以多个平行窗口复制职责：

- **菜单栏主面板**由固定品牌/设置头、独立滚动内容和固定关闭 footer 组成，固定宽 312pt、首选高 560pt。内容负责手动选择默认组／工作区、活动概览、所选组的换包与音量，以及五个事件的试听／静音；不显示字号入口、包管理或第二个设置入口。头部「设置」明确提交 `.destination(.general)`，关闭设置后按 retained handback 恢复该按钮焦点。
...
- **统一设置窗口**沿用仓库内原型 `mockups/ai-app-manager-native-macos.html` 的导航形态，以固定侧栏承载「通用、集成、默认组／工作区、通知、显示、声音、用量、快捷键、关于」九个真实目的页。窗口在 app 生命周期内复用，所有菜单与页间动作只提交类型化路由；关闭后由这一处统一归还焦点。
  - **集成**复用现有 capability、连接、回执与恢复事实，按 Host Product 分组但以 `HostSurfaceID` 为动作目标。
  - **默认组／工作区**展示两类完整声音配置、五事件、试听／自动开关及有效使用状态；缺声深链定位「声音」里的包与事件，只读内置包提供目标明确的复制并应用动作。
  - **声音**迁入完整 Sound Packs 编辑能力，是唯一包级映射写入面；AI 按包执行“描述 → 候选与命名 → 明确采纳”，内部声音方案隐藏。完整集合显示 3 个候选；只有路线政策允许时才显示 1–2 个，并以 `N/3` 明示 partial。包级采用以 `packID + Event` 为目标，选择使用的默认组／工作区是独立动作，不制造按来源分叉的 manifest。
  - **通用、通知、显示、用量、快捷键、关于**不得是占位或演示常量；各自的真实事实源、权限、失败、持久化和验收边界见 `plan/PLAN-SETTINGS-EXPERIENCE.md`。

### AI 提示音候选与生产资格（现行 · 2026-09-13）

**2026-09-17 包级设计覆盖**：ADR 0016 已接受，交互原型为
`mockups/pack-scoped-ai-cues.html`。服务卡放在「声音」顶部，只显示五个已注册固定 profile 与
各自凭据状态；包侧栏固定显示 `n/5`。「声音」页不默认选中事件，事件名称是普通标签；点击
「描述生成」或从缺声深链进入时定位对应事件，在行下方显示生成表单或只读包复制引导。
生成时可见目标包与事件、服务和凭据状态、采纳后受影响的使用者。宿主事件能力由现有
`HostCapabilityBinding` 持有，在「集成」查看来源的原生事件、支持和实现状态；
当前激活由真实回执判断。来源能力不限制包级生成，也不决定默认组／工作区的声音选择。
可直接向健康用户包采纳；「提示音组」是普通用户包的界面称呼，新建空组在首音采纳成功前不入库；
任意健康包复制成功即发布，普通复制只查看副本。缺声深链对只读内置包
提供明确默认组或工作区目标的「复制并用于…」，应用失败保留可找回副本。共用包的有效使用者
是选用该包的默认组／工作区，修改立即生效，范围不完整时可见提示；动作旁提前说明带整包声明的
副本或采纳 AI 后移除修改后包的 `license`/`author`，无确认弹窗。以下候选与生产资格条款
仍有效；旧 scoped 入口的实施状态不得由本设计定稿推断。

2026-09-15 生成交互补充：点击生成后，描述框保持背景、边框与文字样式，但编辑器被可滚动的只读
描述替代，不显示插入光标、不接受键盘/粘贴/输入法修改。状态层独立拒绝生成与采用期间的描述修改。
以可见文字和 VoiceOver hint 说明“生成中，描述已锁定；如需修改，请先取消。”，生成时焦点移到
“取消”；取消或失败保留原描述并恢复编辑。面板内主动取消走“取消”，关闭窗口、切换包/profile、
超时与网络失败仍执行原终止保护。生成完成后的修改描述仍会使未采用候选失效，改名不会重新生成。
...
独立 retained 非激活 `NSPanel` 只在自动到达时 order-front；显式来源、数量或菜单栏
「需要你」动作才进入键盘交互。沿用 SF Pro Rounded、clay、18 / 13 / 11 圆角与事件色，
执行中断保持琥珀色。首行显示宿主和原因；次行只显示已知项目与短 ID，来源缺失集中在详情解释。

外宽约 440pt，两侧至少 16pt，顶部避让可见区与刘海。横幅、列表、详情及空态分别布局；
正常列表每行最小 54pt、五行视口 270pt，第六项起滚动可达。窗口高度受屏幕约束，
...
零项仍可打开，文案为「当前没有保留的待接手提醒」，不表示宿主任务完成。

查看、复制及收起不移除提醒，移除有独立动作。生产只提供详情与有效 ID 复制，复制按实际结果反馈。
Esc 按详情→列表→收起退出；沿用既有焦点所有权归还。声音静默、音量为零、播放失败不删除视觉事件。
动态静默期间保留合格提醒，恢复不补播；锁屏、睡眠、禁用与退出清空来源和在途动作，
锁屏及睡眠全部恢复后才接收。用户主动复制的系统剪贴板不被清空。
...
所选组音量为零、音频缺失和播放失败都不减少回调计数；`stop` 只表示响应结束，不表示任务成功。Panel 与
「活动与诊断」消费同一 `ActivityDiagnosticsModel` 和 `ActivityOverviewProjector`，不允许 View 内另算。

- Panel 的自然 Tab 顺序是 Settings → Sound Scope → 今日/7 日 → 四段活动色条 → 可用事件的试听/静音 → 所选组音量 → 关闭；自动打开可把初始焦点落在 Sound Scope，Settings handback 才显式恢复 Header Settings。
...
- 窗口默认 1240×820、最小 960×640；窗口宽度不超过 1100pt 时侧栏 210pt，否则 252pt，内容最大阅读宽度约 820pt。页面内滚动不得带动整个窗口横向溢出。
...
**试听与静音正交**：事件开关只控制宿主真实事件的自动播放。所选组音量非零且映射文件安全可读时仍可手工试听；音量为零、未映射、文件缺失／损坏或安全／可读性校验失败必须显示明确原因。
...
- **最大内容宽（营销）**：~1060px。
- **行结构（每事件行）**：`[事件色 glyph] · [本地化标题 / claudi0 ID（mono） / 声音文件] · [试听] [静音]`。面板一次显示所选默认组或工作区的五行；Surface 适用信息只读，不作为编辑选择器。声音文件只读，「编辑工作区」定向进入同一设置窗口，逐事件文件编辑进入「声音」目的页。生产只使用固定紧凑行结构，禁止通过字号或宽度偏好改变动作位置。
- **控件行（Control Row）—— 面板里一切非事件的设置行**（所选组音量滑块 · 未来的开关 / 步进器 / 行内按钮）。**这一节管的是既成事实，不是新发明**：`OnboardingView` 主 CTA 早已是「原生外壳 + `.tint(clay)`」，此处只是把它升格成全 App 规则。
  ...
  - **行解剖**：播放设置使用一个统一描边的两行组：所选组音量行与声音包行之间用系统 `Divider`；内边距沿用面板 12–13pt。控件行没有事件色 glyph，这是它与事件行的结构差别。
  - **不加图标（默认）**：五个事件行已各自带试听与静音操作字形。控件行再上一枚喇叭 = 同一个 312pt 面板里的**第三套**音量语义，读者无从分辨哪个才是"音量"。**标签用文字**。确需图标时，须挑一个不与事件行撞的字形并登记在此。
  - **数值读数（现行显示）**：所选组音量行固定显示百分比，使用 SF Mono / `.monospacedDigit()` 与可容纳 `100%` 的定宽槽；同一值继续作为 `accessibilityValue` 播报。
...
### 菜单栏 Agent 集成面板（2026-09-06 历史交互解剖）

以下 Global/Surface 选择器、覆盖数、全局主音量、旧窗口路由及图中的 `Aa⌄` 均只作历史记录；现行声音作用域、操作和尺寸以上方「默认组与工作区」及「全产品界面职责」为准。固定头／滚动区／footer、事件语义色与焦点归还等不依赖旧声音模型的视觉约束仍适用。
...
- **声音作用域焦点与动效例外（2026-09-11）**：本选择器的触发卡、作用域选择按钮和行内
  集成动作共同使用整行交互状态。hover / focus 使用行表面与语义色状态图标回应；焦点还在
  图标上增加约 3pt 语义色光晕，按压任一子按钮时整行缩放至 0.985 后回弹。选中行持续保留
  `clay-soft` 底与 `clay` 描边，交互时只叠加轻微抬升表面；集成动作的 chevron 仅在直接
  hover / focus 时移动。macOS 14+ 在这三个控件上关闭系统焦点效果，由自绘整行状态承载焦点；
  macOS 12–13 保留系统焦点环并同时显示相同自绘状态。此例外只属于声音作用域选择器，其他
  原生 Button、Slider、Toggle 和 Stepper 继续遵守控件行的系统焦点环规则。减弱动态效果时
  保留静态表面、描边与光晕，取消位移、缩放和补间。
...
固定紧凑布局与系统辅助功能缩放、明暗/高对比度、Reduce Motion 与 Transparency 必须另行人工走查；未走查不得标为完成。
...
|------|----------|-----------|
| 2026-09-17 | AI 提示音改为在「声音」中面向用户包生成和采纳；Global/Surface 仅选择使用的包，ADR 0016 取代下行来源独占设计 | 普通包可共享，包级映射的修改本来同时影响所有有效使用者；空组首音成功才发布、复制与深链应用分开，避免隐式切换作用域。候选与 Provider 安全合同保留，Swift 与原生验收另计。 |
