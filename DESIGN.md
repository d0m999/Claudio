...
> **本节是现行方向的单一真相源。**「暖色工具主义」的**战略**不变（暖、五事件五色、`StopFailure` 绝不红、颜色即语义、原生骨架）；糖果盘是它的**现行渲染皮肤**，把「深夜信号仪表」的冷静表面换成更圆润、更有触感的糖果质感。**凡本节与下方 v1 的 token 表 / 对比度注 / 组件几何冲突，以本节为准；v1 值按本文档惯例存档不删**（历史里的对比度推演仍是有效的方法论记录）。全交互原型见顶部预览链接。
...
- **③④ 面板包列表（2026-08-25 废止）**：旧「星标 ≤4 + 覆盖轨」仅作为历史组件与管理窗口资料保留，生产面板不再挂载声音包画廊。当前包改由播放设置组中的单一声音包行呈现；「打开设置」先进入当前 Sound Scope 的「事件与提示音」窗口，逐事件文件编辑再委托 Sound Packs Window。
...
>
> **决议（现行）**：**tile 保持事件色自染 15% 不变**；改为把**两个**亮色事件色调深 —— `Stop` `#2FA24E → #288B43`、`StopFailure` `#C87A00 → #AC6900`。`Notification` `#C4633C`（= clay 品牌招牌绑定）与 `SubagentStop` `#5B59D6` **不动**；**原四个 lifecycle 事件的暗色全部不动**（本来就宽裕）。**原四事件语义一个不改：`StopFailure` 仍是琥珀，绝不用红**；新增 `TaskStart` 独立绑定 info blue —— 调整只涉及 token 明度或新增语义，不混色相。
>
...
| info | `#0A84FF` | `#0A72D0` | 已落地；`taskStartDark/taskStartLight` 明确绑定此 token。真实 15% 自染 tile 上最差 glyph 对比度约 3.60:1，tile 对 panel 约 1.18:1 |
...
> **刻意不复用 `StopFailure` 的 `#AC6900`**：那会把「UI 语义层与事件层刻意分叉」当场合并成同一个 hex，而这条分叉本身就是写下来防手抄的。**原四事件语义一个不改：`StopFailure` 仍是琥珀，绝不用红。**
...
- **基准单位**：4px。
- **密度**：comfortable-compact（菜单栏面板要紧凑但不挤）。普通控件行高 ~28pt；事件行由“最多两行标题 + 一行小标签”自然决定高度，不再用 37/52pt 固定高度裁切内容；面板内边距 12–13pt。
...
- **收起态与浮层选项**：触发卡左侧显示名称与「覆盖数 · 状态」，右侧同时显示语义图标、状态文字
  ...
  焦点离开后收起并把焦点还给触发卡；方向键只在「全局默认 → registry Surface」各选项间移动，
...
- **行内状态动作（2026-09-10，替换菜单底部「连接与诊断…」footer）**：展开层中状态为
  ...
  行内状态动作不进入方向键顺序，只进入 `Tab` 焦点序；同一行两个动作的无障碍 label 必须
  可区分——选择按钮朗读名称与状态摘要，动作按钮朗读「在集成中处理 X」。
...
- **播放设置**：两行使用统一边框。主音量保留 `VolumeDragSession` 的一次提交、失败回滚与关闭前
  ...
  携带 `PanelSoundScopeID` 进入事件窗口，其逐事件编辑路由再携带 `HostSurfaceID?` 与 effective pack 进入
  Sound Packs Window；Surface 下另有定向「重置为全局」。
...
- **焦点顺序**：收起时为 `soundScope → bootstrap/config recovery → 各事件可用试听/静音 →
  ...
  `soundScope` 后插入各可见选项，行内状态动作跟随各自选项只进入 `Tab` 序。
...
- **包行四态 `PackCardState{complete | partial | broken}` × 正交的 `isSelected`（2026-07-15 定态模型 · /plan-design-review；2026-07-17 形制改竖排整宽行 · 用户拍板 mockup）**
  行的**唯一工作是换包**。整宽行 = `[包名 SF Pro 13] [meta 槽 mono 10] [Spacer] [5-slot 覆盖轨]`，`surface-2` 底、radius 10、1px `hairline-strong` 描边、内边距沿用面板 12–13pt。包显示名是 UI 文本走 SF Pro；`name` 缺失回落 mono 的 `id`（id 是数据）。**（⚠ 糖果盘现行：行底改**白色糖果卡** `surface` `#FFFFFF`、radius **13**、包名走 **SF Pro Rounded**，见「现行视觉皮肤：糖果盘」③④；上文 surface-2 / radius 10 / SF Pro 为 v1 存档。行的解剖与语义不变。）**
  - **覆盖轨恒显（含 `complete`）—— 每个可读包的行右侧永远画满 5 格。（2026-07-17 推翻「complete 零图形」，旧决议存档于 Decisions Log 不删）**
  ...
  - **`complete`** —— 5 格全亮（事件色实心）；meta 槽只在 `license == "CC0-1.0"` 时显 `CC0`（`text-2`）。
...
- **5-slot 覆盖轨（由 2026-07-17 的 4-slot 组件扩展；`broken` 行仍以**状态行**替代轨）**
  按 `Event.allCases` 固定顺序的一排 5 格。两个尺寸档：**面板行档** ≈ 18×10pt、**胶囊**（pill 999 档的半高应用 —— mockup 量得的 radius 5 即半高，不是新圆角值）、间距 4pt（从用户 mockup 量得，落地按行宽微调 —— 312pt，`.maximum` 档 360pt）；**管理窗口侧栏微型档** 9×9pt、radius 2（阶梯外微几何，已登记于圆角阶梯行）、间距 3pt。同一组件、同一配色与形状语义，只差几何。几何随 `typeScale` 缩放（旧卡片 `@ScaledMetric` 的既有先例）。
...
- **面板显示集 · 星标〔2026-08-25 废止 / 存档〕**：2026-07-17 的「最多四行星标包 + 全宽管理声音包入口」不再进入生产菜单栏面板。`starred_packs` 与旧 Pack Row 仍保留配置兼容和历史组件，不做 schema 迁移或删除；生产面板仅显示当前 effective pack，并通过「打开设置」进入带 Global/Surface scope 的「事件与提示音」窗口。既有 Decisions Log 保留为历史决策，不得再把它引用为当前 Panel wiring。
...
- **布局**：标准字号下为 `[侧栏 176pt：包列表] | [主区：事件映射]`；辅助功能字号下上下堆叠。详情滚动区之外固定 `[包级动作栏]`，避免长事件/孤儿列表把主动作推走。
  ...
  - **主区**：统一状态区在最上方；详情头为包名 + license / `⚠ 已修改` 标 + `在访达中显示`；下面是**五条事件映射行**，每行 = `[事件名 + id] [文件名下拉 ▾] [试听 ▶]`。状态按单调 revision 仲裁，包级与窗口级结果可同时存活，不让较晚完成的后台动作覆盖另一条失败。
...
|------|----------|-----------|
| 2026-08-26 | AI 提示音进入 scoped「事件与提示音」窗口：页面级 BYOK，逐事件两步生成，候选阶段命名，声音方案隐藏 | 用户的创作输入只应是声音描述；名称属于采用元数据，不应污染 provider 请求。manifest 是 pack-wide，因此只有明确 Surface 的独立用户包能安全承诺“只影响此来源”；其余目标必须可见地 fail closed。 |
| 2026-08-25 | 菜单栏生产面板改为全宽 Global/Surface 作用域菜单 + 当前来源五事件 + 两行播放设置组；「打开设置」进入 scoped 「事件与提示音」窗口，逐事件编辑委托 Sound Packs Window | Surface 声音偏好必须以 Global 与稀疏覆盖为同一条可见主线；事件能力由显式 support/implementation 模型驱动，WorkBuddy 诚实显示 2/5。事件窗口保留 scope，Sound Packs 路由只负责完整音频映射，错误 scope fail closed；继续支持浅色/深色与 312/360pt，并保留配置 schema 与未知字段。 |
