...
顶部事件来源提示是一个独立的、短暂的辅助 presentation surface，不改变“两顶层
surface”的设置与菜单栏职责。它由 app-lifetime receiver、`EventNoticeModel` 和一个
retained 非激活 `NSPanel` 组成：自动到达只 order-front，不 activate Claudio、不 make-key、
不提交输入法组字；只有用户显式点击来源、数量或菜单栏「近期提示」才进入键盘交互。

胶囊约 440pt 宽，顶部距可见安全区 12pt，两侧至少 16pt；沿用 SF Pro Rounded、clay 和
五个事件语义色，保持两条紧凑文字基线。来源、项目和事件优先保留，完整安全标签在详情
列表中呈现。悬停、键盘焦点或展开任一存在时暂停 4 秒阅读计时；连续事件共用一个容器，
新事件不会替换正在阅读的条目。近期只在本次 GUI 内存中保留最多 50 条、最长 30 分钟，
不进入 receipt、receipt history、活动摘要、日志或配置；过期、应用关闭、锁屏、睡眠和
退出会清除来源与导航目标。用户显式收起胶囊只改变展示状态，近期记录仍可从近期入口
重新查看，不将收起表示为已处理。

事件静默、主音量为零或播放失败都不删除视觉事件。ADR 0009 的 Focus/Calendar 快照有效
时只收起自动提示，恢复后不补播；偏好关闭则立即停止来源接收、清空内存并收起窗口。
没有宿主 adapter 证据的精确会话 route 不显示伪成功，只提供来源详情和复制会话 ID。
