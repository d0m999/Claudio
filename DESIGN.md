...
## 默认组与工作区（现行 · 2026-09-24）

本节替代下文历史 Global/Surface 声音选择、稀疏覆盖、唯一全局主音量与按声音作用域统计活动的规则；字体、主题、固定 312pt 面板宽度与单一 retained 设置窗口继续有效。
...
- **统一设置窗口**对应仓库内原型 `mockups/ai-app-manager-native-macos.html`，以固定侧栏承载「通用、集成、事件与提示音、通知、显示、声音、用量、快捷键、关于」九个真实目的页。窗口在 app 生命周期内复用，所有菜单与页间动作只提交类型化路由；关闭后由这一处统一归还焦点。
  ...
  - **事件与提示音**保留 Global/Surface、五事件、试听/静音和 effective profile；缺声「去生成」深链定位「声音」里的包与事件，只读内置包显示明确目标的复制并应用动作。
  - **声音**迁入完整 Sound Packs 编辑能力，是唯一包级映射写入面；AI 按包执行“描述 → 候选与命名 → 明确采纳”，内部声音方案隐藏。完整集合显示 3 个候选；只有路线政策允许时才显示 1–2 个，并以 `N/3` 明示 partial。Global 写顶层 `selected_pack`，Surface 写稀疏覆盖，不制造 per-surface manifest 副本。
...
`HostCapabilityBinding` 持有，在「事件与提示音」按当前来源查看原生事件、支持和实现状态；
当前激活由真实回执判断。不支持或未实现的事件不提供缺声入口，来源能力不限制包级生成。
可直接向健康用户包采纳；「提示音组」是普通用户包的界面称呼，新建空组在首音采纳成功前不入库；
任意健康包复制成功即发布，普通复制只查看副本。缺声深链对只读内置包
提供明确 Global/Surface 的「复制并用于…」，应用失败保留可找回副本。共用包的有效使用者
包含继承 Global 的来源，修改立即生效，范围不完整时可见提示；动作旁提前说明带整包声明的
