...
> 对应 ENGINEERING.md「v1 新增/加固决议 · 客观声音质量标准」+ 任务 T9："客观声音质量标准：单音时长上限（≤~2s）、响度归一化目标（峰值 -1 dBFS 或 -16
> LUFS）、峰值限制、静音头尾裁剪、**五事件**差异原则（音色/音高或时序有别，任两两之间共 C(5,2)=10 对）；`master_volume`(0.0–1.0)→`afplay -v` 映射含默认值与越界钳制。"
>
...
| 推荐目标（默认，非硬性） | ≤ 1.2s；普通包的 `task_start` 为 **220–320ms** | 同上；五个事件里任一个逼近 2.0s 硬顶都建议重剪，高频事件（`task_start` / `subagent_stop`）默认尤其应短 |
| 长尾疗愈例外（需在声音包台账中明确声明） | `task_start` 总时长可放宽至 **≤ 1.2s**；前 320ms 必须完成清晰的启动触发，后段只能是更低电平的自然共鸣尾韵 | 仅适用于明确定位为冥想、共鸣或低扰动的策展包；仍受 2.0s 硬顶、峰值和重叠可用性约束，尾部应使用连续自然释放而非边界硬切，并在释放后保留 ≤80ms 的明确数字静音保护区；不改变其它声音包的默认时长要求 |

理由：hook 是 Claude Code 响应路径上的同步调用，`claudio play` 本身立即返回（不等 afplay 退出，见 `Play.swift`），但用户体验上
声音仍需在合理时间内"说完"，且 ENGINEERING.md「并发 / 进程堆积处理」的去抖窗口是 1.5s——普通包的单音明显长于去抖窗口会让"这次到底有没有响"更难判断。长尾例外必须把启动辨识放在前 320ms，把后段控制在低电平并使用连续自然释放；释放完成后还要留出一小段数字静音，避免有损编码的尾部残留，接受连续触发时的尾韵重叠作为明确的风格取舍。

## 2. 响度归一化目标

旧四个 lifecycle 事件二选一并使用同一种目标（不可随意混用峰值法和 LUFS 法，否则事件之间的主观响度会不一致）。`task_start` 是高频、前置确认音，策展包允许刻意更轻：最终解码峰值必须**不高于 -6 dBFS**，并与其余四音人工比对，不能靠自动峰值替代听感判断。
...
| 长尾例外的终点 | 自然释放必须连续落到零；其后的明确数字静音保护区 **≤ 80ms**，最终解码音频的保护区采样值必须全为 0 | 生成 WAV 与最终分发文件解码为 PCM 后，分别检查保护区的 `peak == 0`；不能只检查最后一个采样点 |

## 5. 五事件音色·音高与时序差异原则

五个事件（task_start / stop / stop_failure / notification / subagent_stop）**任两两之间**（共 C(5,2) = 10 对），必须在音高、音色或时序轮廓上**至少一项**
可辨——满足下列任一判据即通过：
...
定性锚点（呼应 DESIGN.md「动效跟随音高轮廓」——非可自动化的数值项，仅作设计意图人耳复核依据）：**TaskStart = 简短上扬或双确认、Stop = 上扬两音、
StopFailure = 下沉且保持、Notification = 双击、SubagentStop = 单 blip**——五个事件的**时序包络形状**本身就应彼此不同；数值判据
...
|---|---|---|
| 1 | 单音时长 | ≤ 2.0s（推荐 ≤ 1.2s） |
| 2 | 响度归一化 | 旧四个 lifecycle 音：峰值 -1.0 dBFS ±0.3dB **或** 积分响度 -16 LUFS ±1LU（同一种方法）；普通包 `task_start`：最终解码峰值 ≤ -6 dBFS + 人工响度比对；长尾例外还须保证后段低于启动主体 |
| 3 | 真峰值限制 | ≤ -1.0 dBTP，且无采样点达到 0 dBFS |
| 4 | 静音头尾裁剪 | 头部 ≤ 50ms；尾部 ≤ 80ms；普通音淡入 8ms / 淡出 30ms；长尾例外还须自然落零并验证末尾保护区全零 |
