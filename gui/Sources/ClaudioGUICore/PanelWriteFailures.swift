...
/// （静音 / 切包，见 `PanelView.swift:560-564`），Step 3 落地的 `SetMasterVolumeError` 即将成为第三条。三条并列
/// 的 `if let` 各自只回答「这一个该不该显示」，从没人回答过「两条同时非 nil、且文案逐字相同（三个写者共享同一份
/// `config.json` / 同一把 `config.lock`，同时撞上 `.lockBusy` 不是理论情形）时该显示一遍还是两遍」。这个纯函数
/// 把「该不该显示 / 显示几遍 / 显示顺序」从三段并列 `if let` 收拢成一个可测的决策点，面板侧只渲染这一个列表。
///
/// ## 三条不变式
///
/// - **互不顶替**：多条失败可以同时存在（一次失败的静音 + 一次失败的切包），全部保留——`PanelView.swift:546-548`
///   那条注释逐字写着「两条都可能同时非 nil……所以两条都渲染，不互相顶替」。
...
/// `toggleMute`（未来主音量的写路径同理）在这个失败上会把 `configState` 重路由到 `.needsPack`，「先选包」
/// 空态卡本身就是给用户的解释。再把 description 印一遍是重复且从未 QA 过的字符串。`UseError`（切包）从不
/// 产生这个 case——`selectPack` 是全仓唯一手上握着真实 pack id、因而有资格从无到有建出一份 config 的写者。
