...
public enum ClaudioColorHex {
    ...
    /// 事件字形的亮色琥珀被调深到 `#AC6900` 以过它那块自染复合底的 ≥3:1，而 UI `warning` 仍是 `#E08600`。
    /// v1 还没有任何视图渲染它（校验提示面还没落地）——它留在这张表里，是为了这条分叉被记在**一个**
    /// 地方，并且将来第一个用它的视图是从这里取值，而不是又手抄一遍 DESIGN.md。
    /// ⚠️ 亮色 `#E08600` 对亮面板只有 2.73:1：**将来若把它当正文用，必须先过对比度这关**
    /// （所以 `ContrastSuite` 里没有给它开 ≥4.5:1 的文本断言——今天它不是文本）。
    public static let warningDark = "FF9F0A"
    public static let warningLight = "E08600"
    ...
    /// 它因此也不再等于 UI 语义色 `warning`（仍是 `#E08600`），见 `warningLight`。
