...
func runPanelFocusOrderSuites() {
    ...
    suite(
        "panelFocusOrder: onboarding .installed shape (no primary, only secondary) → secondary is first focus"
    ) {
        ...
        expect(
            order.first == .onboardingSecondaryAction,
            "first focus must be the only actionable control")
    }
    ...
    suite(
        "panelFocusOrder: operational — each row contributes eventSound THEN action THEN mute, in Event.allCases order (follows visual left-to-right order, PLAN-SOUND-MANAGER.md §2.5/T2's 3-slot row)"
    ) {
    ...
    suite(
        "panelFocusOrder: operational — first focus is the first row's file-name Menu (eventSound sits leftmost, visually first; mute sits rightmost, visually last)"
    ) {
        let order = panelFocusOrder(
            .operational(events: Event.allCases, packCardIDs: [], hasMasterVolume: true))
        expect(
    ...
    suite(
        "panelFocusOrder: operational — gallery cards come right after the master volume slider, then 管理声音包, with 退出应用 last"
    ) {
        let order = panelFocusOrder(
            .operational(
                events: Event.allCases, packCardIDs: ["alpha-pack", "zeta-pack"],
                hasMasterVolume: true))
    ...
    suite(
        "panelFocusOrder: .masterVolume's position is pinned — right after the last row's .eventMute, right before the first pack card"
    ) {
        let order = panelFocusOrder(
            .operational(events: Event.allCases, packCardIDs: ["alpha-pack"], hasMasterVolume: true)
        )
    ...
    suite(
        "panelFocusOrder: operational — total count is 3×events + masterVolume + cards + manageSounds + quitApplication"
    ) {
        let order = panelFocusOrder(
            .operational(
                events: Event.allCases, packCardIDs: ["a", "b", "c"], hasMasterVolume: true))
    ...
    suite("panelFocusOrder: onboarding vs operational produce structurally different orders") {
        let onboardingOrder = panelFocusOrder(
            .onboarding(hasPrimaryAction: true, hasSecondaryAction: false))
        let operationalOrder = panelFocusOrder(
            .operational(events: Event.allCases, packCardIDs: [], hasMasterVolume: true))
        expect(
            onboardingOrder != operationalOrder,
            "the two scopes must never coincidentally produce the same order shape")
        expect(
            !onboardingOrder.contains(where: {
                if case .eventMute = $0 { return true }; return false
            }),
            "onboarding's order must never contain an operational-only target")
    }

    suite(
        "panelFocusOrder: an empty operational panel (no cards, hasMasterVolume: false, no config-failure notice) is [.manageSounds, .quitApplication]"
    ) {
    ...
    suite(
        "panelFocusOrder: an empty-rows operational panel with hasMasterVolume: true still surfaces the slider ahead of quit"
    ) {
        ...
        let order = panelFocusOrder(
            .operational(events: [], packCardIDs: [], hasMasterVolume: true))
        expect(
    ...
    suite(
        "panelFocusOrder: the config-failure shape (.malformed/.unwritable, no cards) leads with .configReveal, then .manageSounds, then .quitApplication"
    ) {
    ...
    suite(
        "panelFocusOrder: the config-failure shape WITH pack cards leads with .configReveal, then cards, .manageSounds, and .quitApplication (never folds into the plain .needsPack card order)"
    ) {
    ...
    suite(
        "panelFocusOrder: quit appears exactly once and is always last in every operational shape, never onboarding"
    ) {
    ...
    suite(
        "panelFirstFocusTarget: nothing disabled → first focus is the first row's file-name Menu (eventSound), same as order.first"
    ) {
        let scope = PanelFocusScope.operational(
            events: Event.allCases, packCardIDs: [], hasMasterVolume: true)
        expect(
    ...
    suite(
        "panelFirstFocusTarget: first row's action disabled (muted present) → first focus is STILL that row's eventSound — the disabled action is never even reached"
    ) {
    ...
    suite(
        "panelFirstFocusTarget: the full order (incl. the disabled action) is unchanged by disabling it — the disabled action remains a Tab stop, only opening focus is unaffected by it"
    ) {
        let first = Event.allCases[0]
        let scope = PanelFocusScope.operational(
            events: Event.allCases, packCardIDs: [], hasMasterVolume: true)
        ...
        expect(
            fullOrder == expected,
            "the full order (incl. the disabled action) must be unchanged, got \(fullOrder)")
        expect(
            fullOrder.contains(.eventAction(first)), "the disabled action must remain a Tab stop")
    }

    suite(
        "panelFirstFocusTarget: nonOperableActionEvents no longer moves OPENING focus at all — empty / first-only / non-first / every event all resolve to the SAME first row's eventSound"
    ) {
        let scope = PanelFocusScope.operational(
            events: Event.allCases, packCardIDs: [], hasMasterVolume: true)
    ...
    suite(
        "panelFirstFocusTarget: onboarding scope ignores nonOperableActionEvents (it has no action targets)"
    ) {
    ...
    suite(
        "panelFirstFocusTarget: empty operational panel, hasMasterVolume false, no cards, no config-failure notice → first focus stays .manageSounds ahead of quit"
    ) {
    ...
    suite(
        "panelFirstFocusTarget: the config-failure shape (.malformed/.unwritable, no cards) → first focus is .configReveal, never 断开连接 (26bba37 follow-up)"
    ) {
    ...
    suite(
        "panelFirstFocusTarget: the config-failure shape WITH pack cards → first focus is STILL .configReveal, never the first pack card (the pre-fix bug: reveal button was skipped to the gallery)"
    ) {
    ...
    suite(
        "panelFirstFocusTarget: empty operational panel with pack cards (the COMMON .needsPack 'pick a pack' shape) → first focus is the first pack card, not 断开连接"
    ) {
    ...
    suite(
        "panelFirstFocusTarget: empty operational panel, hasMasterVolume true → first focus IS the master volume slider (never nil)"
    ) {
    ...
    suite(
        "panelOpeningFocus: nothing muted → opening focus is the first row's eventSound (agrees with order.first)"
    ) {
        let rows = Event.allCases.map {
            EventRow(event: $0, coverage: .present(fileName: "\($0.cliName).mp3"), enabled: true)
        }
        let target = panelOpeningFocus(
            rows: rows, packCardIDs: ["alpha-pack"], hasMasterVolume: true)
        expect(
        ...
        expect(
            target
                == panelFocusOrder(
                    .operational(
                        events: rows.map(\.event), packCardIDs: ["alpha-pack"],
                        hasMasterVolume: true)
                ).first,
    ...
    suite(
        "panelOpeningFocus: a MUTED .present first row → focus is STILL that row's eventSound (mute never reaches, let alone disables, the file-name Menu)"
    ) {
        let first = Event.allCases[0]
        ...
        rows[0] = EventRow(
            event: first, coverage: .present(fileName: "\(first.cliName).mp3"), enabled: false)
        ...
        let order = panelFocusOrder(
            .operational(events: rows.map(\.event), packCardIDs: packCardIDs, hasMasterVolume: true)
        )
        expect(
            target == order.first,
    ...
    suite(
        "panelOpeningFocus: an UNMAPPED muted first row's opening focus is ALSO its eventSound (the row's actually-fixable, always-operable control — mute is irrelevant to reaching it)"
    ) {
        let first = Event.allCases[0]
        var rows = Event.allCases.map {
            EventRow(event: $0, coverage: .present(fileName: "\($0.cliName).mp3"), enabled: true)
    ...
    suite(
        "panelOpeningFocus: every row muted+present → focus still lands on the FIRST row's eventSound (never a later row, never the disabled action, never the mute)"
    ) {
        let rows = Event.allCases.map {
    ...
    suite(
        "panelOpeningFocus: every row muted → first focus is the first row's eventSound, NEVER the master volume slider (the slider only wins opening focus when the row list is empty, see the hasMasterVolume-gated tests below)"
    ) {
        let rows = Event.allCases.map {
            EventRow(event: $0, coverage: .present(fileName: "\($0.cliName).mp3"), enabled: false)
        }
    ...
    suite(
        "panelOpeningFocus: zero rows, hasMasterVolume false, with a pack card (the REAL .needsPack 'pick a pack' shape) → first focus is the first pack card, never the (unrendered) slider or a removed drop zone"
    ) {
        ...
        expect(
            panelOpeningFocus(rows: [], packCardIDs: ["alpha-pack"], hasMasterVolume: false)
                == .packCard(id: "alpha-pack"),
    ...
    suite(
        "panelOpeningFocus: zero rows, hasMasterVolume true → the master volume slider, never nil"
    ) {
        ...
        expect(
            panelOpeningFocus(rows: [], packCardIDs: ["alpha-pack"], hasMasterVolume: true)
                == .masterVolume,
...
func runPanelFocusInFlightSuites() {
    ...
    suite("panelOpeningFocus: 断开跑到一半时，禁用的「断开连接」不能拿首焦点（operational）") {
        ...
        expect(
            panelOpeningFocus(
                rows: rows, packCardIDs: [], ctaOperable: false, hasMasterVolume: true)
