...
func runPanelConfigSuites() {
    suite(
        "loadPanelConfig: a missing config.json is .needsPack — not an error, the self-heal path (picking a pack) is open"
    ) {
    ...
    suite(
        "loadPanelConfig: a corrupt config.json is .malformed with an actionable reason — never .needsPack (D23: these two used to be the same empty-pack fallback, hiding that self-heal is NOT open here)"
    ) {
        withTempDirectory { root in
    ...
    suite("loadPanelConfig: selected_pack is an empty string is .needsPack, same as a missing file")
    {
        withTempDirectory { root in
            ...
            let state = loadPanelConfig(from: configFile)
            expect(
                state == .needsPack, "an empty selected_pack must report .needsPack, got \(state)")
        }
    }
    ...
    suite(
        "loadPanelConfig: content is fine but the parent directory is read-only → .unwritable, not .operational"
    ) {
        ...
        withTempDirectory { root in
            ...
            guard case .unwritable(let reason) = state else {
                expect(
                    false, "a config whose directory is read-only must be .unwritable, got \(state)"
                )
                return
            }
    ...
    suite("loadPanelConfig: 一次刷新只消费一份有界配置快照，磁盘在读取后变化也不混合三份结果") {
        withTempDirectory { root in
            let configFile = root.appendingPathComponent("config.json")
            let first = Data(
                #"{ "selected_pack": "first-pack", "master_volume": 0.25, "events": { "stop": false } }"#
                    .utf8)
            let second = Data(
                #"{ "selected_pack": "second-pack", "master_volume": 0.75, "events": { "stop": true } }"#
                    .utf8)
    ...
    suite(
        "PanelConfigState.topContent: 单一分类——.operational→.events, .needsPack→.needsPack, .malformed/.unwritable→.configFailure，且 .malformed 与 .unwritable 折叠成同一张失败卡并原样带上 reason"
    ) {
        expect(
        ...
        expect(
            PanelConfigState.malformed(reason: "bad json").topContent
                == .configFailure(reason: "bad json"),
            "`.malformed` 必须映射成 `.configFailure` 并**原样**带上 reason —— 去重使用原始原因，失败卡显示分类文案与"
                + " 焦点 hasConfigFailureNotice 都读这个映射，got"
                + " \(PanelConfigState.malformed(reason: "bad json").topContent)")
        expect(
            PanelConfigState.unwritable(reason: "dir 0500").topContent
                == .configFailure(reason: "dir 0500"),
    ...
    suite(
        "PanelTopContent.showsEventContent / .hasConfigFailureNotice：两颗焦点投影的返回值——.events 独有 showsEventContent，.configFailure 独有 hasConfigFailureNotice，其余全 false"
    ) {
        expect(
            PanelTopContent.events.showsEventContent,
            "`.events` 必须 showsEventContent（= hasMasterVolume 真：滑块 + 五行事件在屏幕上）")
        expect(
            !PanelTopContent.needsPack.showsEventContent,
            "`.needsPack` 不得 showsEventContent —— 先选包空态没有滑块")
        expect(
        ...
        expect(
            !PanelTopContent.events.hasConfigFailureNotice, "`.events` 不得 hasConfigFailureNotice")
        expect(
            !PanelTopContent.needsPack.hasConfigFailureNotice,
            "`.needsPack` 不得 hasConfigFailureNotice")
    }
}
