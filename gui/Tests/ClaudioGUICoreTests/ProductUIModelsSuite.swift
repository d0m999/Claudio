...
func runProductUIModelsSuites() {
    suite("显示偏好删除：Panel 使用单一固定紧凑布局") {
        expect(
            standardPanelWidth == 312
                && panelLayoutAdaptation().panelWidth == standardPanelWidth
                && !panelLayoutAdaptation().hidesWaveform,
            "生产面板必须只有 312pt 的紧凑布局事实")
    }
