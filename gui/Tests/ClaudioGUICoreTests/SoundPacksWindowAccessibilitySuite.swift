...
func runSoundPacksWindowAccessibilitySuites() {
    ...
    suite("SoundPacksWindow a11y：新窗口 target 不得耦合任何面板专用 a11y 类型") {
        ...
        if let data = try? Data(contentsOf: settingsControllerURL),
            let source = String(data: data, encoding: .utf8)
        {
            scannedSources.append(
                (
        ...
        expect(
            ...
            "应覆盖 Core a11y 设施、Settings owner 及嵌入 view/bridge")
        expect(
        ...
        guard
            ...
            expect(false, "嵌入 Sounds a11y 接线检查缺少 view、Settings owner 或 bridge")
            return
        }

        expect(
            view.contains(".disabled(activeSounds.packs.isEmpty)")
            ...
            "纯焦点序必须绑到 owner 投影的原生列表与 Finder capability，空列表必须禁用")
        expect(
