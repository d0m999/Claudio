...
静音、切包、主音量三个 config 写者的失败在面板上呈现的**一个**有序去重列表：互不顶替、顺序稳定（静音 → 切包 → 主音量）、按类型化原因（typed reason）去重，文案不承担身份。Surface 声音问题的失败行附在其后。它只覆盖「写失败」语义域；刷新失败等「读失败」不进入此列表。
...
当前声音作用域内，由显式 Surface 覆盖损坏或 Surface 配置写失败产生的单一问题；它暂停相关操作并显示失败提示。ADR 0017 的目标契约是：只有结构性损坏类可在有效读回确认覆盖恢复后自动清除；写失败类不凭读回自动清除，既有写成功、重置成功与切换作用域的清除路径仍保留。
