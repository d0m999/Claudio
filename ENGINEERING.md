...
- [x] **T13 (P3, human: ~0.5d / CC: ~30min)** — helper — 版本兼容记录 + uninstall 识别历史命令格式
  ...
  - swift-reviewer 收尾发现 3 项，**orchestrator 已全部修复并各自补测**（改动后 helper **489** 绿 / gui **232** 绿，两包清理重建 0 warning/0 error）：
    ...
    - 反之，reviewer 亲手用**链接真实 `ClaudioCore` 产品**的一次性 SwiftPM harness 逐条验证并**判定通过**的项：`install`/`detect` 对"只含历史路径条目"的配置正确报 `notInstalled` 并追加 canonical 条目（幂等性未被 legacy 集污染）；semver 解析对空串/非数字/溢出/多行输出全部安全返回 `nil`；`DoctorReport.hasFailure` 不受两项版本检查影响；超时确由独立后台队列读 stdout 支撑、不依赖"`claude --version` 输出很短"这一巧合；无第二套并行匹配实现；未虚构任何"历史命令格式"（已对 T13 前 git 历史核对）。
