...
- **写入者有四个，但写路径只有一条**（2026-07-11 `/ship` 收口；2026-07-14 阶段 D 补入第三个写入者；T16 补入星标写者）：`selectPack`（`claudio use` / 画廊切包）、`setEventEnabled`（GUI 静音钮）、`setMasterVolume`（GUI 主音量滑块，`MasterVolume.swift`）与 `setStarredPacks`（管理窗口星标，`StarredPacks.swift`）**都**调 `ConfigMutation.swift` 的 `updateConfigJSON(at:onMissing:mutate:)` —— 同一把非阻塞 `~/.claudio/config.lock`（`ClaudioPaths.configLockFile`）、同一次原子写、同一套「只覆盖调用方自己拥有的那个键、其余顶层键（含未知键）逐字保留、读不懂即 fail-closed」的语义。
  ...
  - **面板对 config.json 的完整判定 + 三档刷新路由（2026-07-13/14 阶段 A′ 收尾 · 红队 9cccc9c 落地）**：`PanelConfigState`（`ClaudioGUICore/PanelConfig.swift`）取代了 `loadPanelConfig` 此前「读不出来就无条件回落成空选择 config」的旧做法——**读**（`packSelection(configFile:)`，`ClaudioCore` 新增的只读探针：只答 config.json 有没有记着一个已选包，不摸包目录/manifest）与**写**（既有的 `probeConfigRewritable(configFile:)`）两条正交轴合成同一个面板判据：`.operational(ClaudioConfig)` / `.needsPack`（还没人选包，自救路径畅通——点一张包卡就走 `selectPack`）/ `.malformed(reason:)`（内容本身坏）/ `.unwritable(reason:)`（内容合法但父目录写不进去）。少一半轴会把用户导向骗人的态：`selected_pack` 合法但 `master_volume` 是字符串的 config，只看读轴会判「已选」，实际每次点击都会写失败。`PanelConfigController`（`ClaudioGUICore`，可实例化 `@MainActor` `ObservableObject`）把原先住在 `PanelView` `@State` 里的 `configState`/`config`/`eventRows`/`packCards`/`packSwitchError`/`muteError`，连同 `toggleMute`/`switchPack`/`reload`/`reloadConfigOnly`，整体搬进这个可 `import` 的类型——是本文件下方「`ClaudioGUI` 零测试覆盖」那段「把视图层拆成可 import 的 library target」那条 TODO 的**部分兑现**：搬的是 config 读模型 + 写路径，视图渲染本身仍留在 `PanelView.body`（那根接线仍由 `ViewWiringSuite` 的存在性检查兜底，见 TODOS）。`PanelConfigControllerSuite` 因此能 `new` 一个真 controller、喂真磁盘 config、断言磁盘字节与读模型确实同步变化，不再靠文本绊线。`PanelRefreshRoute`（`.noRefresh`/`.configOnly`/`.full`）是静音/切包/主音量写盘之后「该做哪一档刷新」的穷尽判定（`panelRefreshRoute(muteSucceeded:error:)` / `packSwitchRefreshRoute(after:)` / `masterVolumeRefreshRoute(succeeded:error:)`，三个 switch 漏 case 编译不过）：`.noRefresh` 只发给可证明什么也没揭示的 `.lockBusy`/`.invalidPackID`；`.configOnly`（静音成功 + 大多数失败的默认兜底）只重读 config + 探一次目录，不扫包库；`.full` 只留给包库本身可能已经陈了的 `.configMissing`/`.packNotFound` 两格——目的是避免每次静音一个事件都在主线程上重扫两个包根 + 读全部 manifest。**视觉未跟上**：`.malformed`/`.unwritable`/`.needsPack` 这三张诚实失败/空态卡目前只有实现，DESIGN.md 与本文件上方交互状态覆盖表都还没登记它们的视觉规格；`StateGalleryView` 也不渲染整面板帧，故这三态在仓库内**零视觉真相源**（TODOS 已跟踪，见「StateGalleryView 没有整面板（PanelView）帧，路由态零仓库内视觉验证」一节）。
  - **共享声音包读模型（2026-08-06）**：composition root 创建唯一 app-lifetime `SoundPackLibrary` actor，同时注入 `PanelConfigController` 与 retained `SoundPacksWindowModel`。actor 只拥有包目录、manifest、coverage、按需音频 inventory 与出厂完整性这些磁盘事实；选择、星标、静音、主音量和面板显示集始终从最新 config 投影，因此纯 config 写不触发扫描。阻塞目录/文件读取在两条专用串行 utility queue 执行（library scan 与 selected inventory 分离）；100 包快照不携带完整目录清单，成功 inventory 只进入四项 fingerprint-keyed LRU，失败不负缓存。首次显示 loading，已有快照时 stale-while-refresh，失败保留 previous 并提供重试。每包 fingerprint 绑定目录、manifest、当前事件声明音频的 device/inode/mode/size/纳秒 mtime+ctime，并用读取前后相同 fingerprint 保证 facts 与文件身份属于同一稳定区间。app 内写在同步 mailbox 边界失效，ready/failed 发布与 invalidation revision 原子排序；扫描中的观察或失效合并为一次 follow-up，同一次跨界面写只由写入方发扫描，另一侧订阅共享结果。外部修改在重新展示或 app 激活时收敛。设计上限与实测见 `docs/adr/0001`–`0004`、`docs/performance/sound-pack-library-benchmark.md`；100 包 M1/APFS/Release 首次 p95 `173.893 ms`、production model 缓存展示 p95 `0.282 ms`，benchmark peak footprint 约 `14.3 MiB`。
  - **Release 体积门禁（2026-08-06）**：helper 的兼容入口 `Contents/Resources/bin/claudio` 改为指向 `claudi0` 的相对符号链接，不再重复携带同一个 Mach-O；GUI/helper 在签名前执行 `strip -x`。`scripts/check-release-size.sh` 以每架构 GUI `4,000,000 B`、helper `2,500,000 B`、其余正规文件总计 `1,000,000 B` 为独立预算，并验证两者架构集合一致及 legacy alias 形状。arm64 可执行 payload 从 `14,365,224 B` 降至 `4,867,696 B`（减少 66.1%），完整 arm64 bundle 为 `5,170,981 B`；详细基线见 `docs/performance/release-size-budget.md`。
