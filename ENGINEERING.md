# Design: Claudio — Claude Code + Codex 双宿主语义化声音中心 (macOS)
...
Claudio 管的是四种稳定的**声音语义**，不是把某个宿主的原生事件名扩散到 UI。`Event` 只保留文件键；宿主事件名与支持级别属于 adapter：

| Claudio 语义 | manifest / CLI 文件键 | Claude Code | Codex |
|---|---|---|---|
...
- `HostCapabilityBinding`：宿主原生事件、Claudio `Event` 与 `supported / partial / unsupported` 的唯一映射表。
...
```text
claudio integrations status [--json]
claudio integrations connect <claude-code|codex>
claudio integrations disconnect <claude-code|codex>
claudio hook <host> <native-event> --installation-id <uuid>
```
...
- `claudio play <event>` 保留为 legacy 入口，其全局 1.5 秒去抖语义不变。新 `hook` 使用**宿主级**去抖状态：Claude Code 与 Codex 不得互相吞掉 1.5 秒内的真实事件。
- `claudio install` / `uninstall` 保持 Claude Code 兼容别名；`claudio setup` 保持原有 CLI 行为（shared bootstrap + Claude Code legacy-compatible connection）。若 Claude Code 已有完整、健康的现代连接，`install` / `setup` 必须成功 no-op；遇到部分、冲突、畸形、旧 helper 路径或 modern + legacy 混装状态则 fail closed，并引导用户用 `claudio integrations connect/disconnect claude-code` 显式修复，绝不能再追加一条可能重复播放的 legacy 链。GUI 首次启动只做 shared bootstrap，宿主连接由用户分别选择。
...
- `CodexIntegrationAdapter` 只操作 `~/.codex/hooks.json` 中 Claudio 自己的 `Stop`、`PermissionRequest`、`SubagentStop` command hook。它不接管 `notify`，不读写私有 trust hash，保留第三方 hook、matcher 与数组顺序。
- Codex 写入成功后状态是 `configuredAwaitingActivation`，固定文案「Claudio 已写好，等待 Codex 确认」。用户须在 `/hooks` 确认；只有当前 installation ID 的首个真实事件回执才能进入 `connectedObserved`。
- 已知旧版 `codex-notify` wrapper 只有在与 Claudio 生成版本**精确匹配**时才可显式迁移 Claudio 分支，并保留原 notifier。未知或修改过的 wrapper fail closed，不能再装一条可能重复发声的 `Stop` hook。该 wrapper 的 `Stop` 分支可能在用户完成 `/hooks` 信任前运行，因此即使写出当前 installation 的 `Stop` 回执也**不能**作为 Codex 激活证据；绿色连接必须由 composable hooks 中当前 installation 的 `PermissionRequest` 或 `SubagentStop` 真实回执点亮。
- `disconnect` 只删除所选宿主中 Claudio 自有条目；保留另一宿主、第三方配置、shared runtime 与声音包。
...
回执路径为 `~/.claudio/integrations/receipts/<host>/<event>.json`，文件权限 `0600`。schema 只允许：schema 版本、installation ID、宿主、原生事件、Claudio 语义事件、时间与脱敏播放结果。
...
产品名：**Claudio**。名字保留项目的历史来源，但现行品牌独立于任一宿主；黏土色、图标与声音语言都属于 Claudio。
...
- **现行一句话**：`brew install --cask claudio`（或下载 DMG 按指引打开）→ 开 app 完成 shared bootstrap → 分别连接 Claude Code / Codex → 选内置包 → 两个宿主按各自真实能力自动响，用户无需手改宿主 JSON。
...
- 两个 adapter 的 connect/disconnect 都幂等，只改 Claudio 自有条目；Codex `notify`、trust、第三方 hooks 与顺序保持不变。
...
- **现行接管机制**：helper-CLI 随 app 安装；GUI 首次运行只执行 `SharedRuntimeBootstrap`，不会静默改写任何宿主配置。用户在 `IntegrationsWindow` 分别连接 Claude Code 与 Codex；Codex 写入后还须在 `/hooks` 确认并以真实回执激活。`claudio setup`、`install`、`uninstall` 保留 Claude Code v1 兼容语义，供旧脚本使用，不代表 Codex 已连接。
...
- Claude Code hooks（`~/.claude/settings.json`）与 Codex composable hooks（`~/.codex/hooks.json`）；Claudio 不接管 Codex 单命令 `notify`。
...
- **双宿主增量**：Claude Code `4/4` 与 Codex 严格 `3/4`、Codex `StopFailure`/未知事件拒播、`PermissionRequest` 的「仅授权请求」、两宿主互不吞事件、配置/断开只改 Claudio 自有条目、`notify`/trust/第三方 hooks 不变、当前/旧 installation 回执与迟到回调、双 adapter 并行状态、`AudibilityMatrix` 变异自证均是自动化 DoD。
