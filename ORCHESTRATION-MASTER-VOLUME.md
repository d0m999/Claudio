...
**Scope**: all
**基线**: `9c33dac`（计划写的 `df843b8`、编排初稿写的 `f00521e` **都已过期** —— f00521e 之后还有 `560c9f3`（台账修复）与 `0aab69a`（PanelView / ViewWiringSuite 的假事实订正）两个 commit，本文档自身即 `9c33dac`。X1–X5 的快照实际是在 `0aab69a` 之上测的）
...
| **X6** | **WIP 参考分支 `cbc02f0` 与本编排不同底。** 它基于 `9162266`（pre-T17），**不含 f00521e**。它已经写过一版阶段 A 的锁分离，但做法与 Step 1 的硬要求**相反**：它**保留了 `lockFile` 旧名**（`cbc02f0:helper/Sources/ClaudioCore/Paths.swift:77`），只新增 `configLockFile`(:111) / `settingsLockFile`(:122)；而 Step 1 明令「**改名**为 `playLockFile`（改名，不是加注释）」。且它那棵树没有 `OnboardingActions.swift`，天然漏掉第 11 个默认值点 | **Step 1 不要参考它的 `Paths.swift`。** 它还写过 Step 3/4/7 要「新建」的**全部三个文件**（`MasterVolume.swift` 117 行、`VolumeDragSession.swift` 173 行、`MasterVolumeController.swift` 56 行）—— 后者文档从未提及 | Step 3 / Step 4 / Step 7 的 prompt 已各自写明该参考什么、该无视什么 |
...
**工作树今日干净**（实测 `git status --short` 为空，HEAD = `9c33dac`）。开工前仍先 `git status` 确认，然后**从 `9c33dac`（即当前 main）拉分支 —— 不要从 `f00521e`**：f00521e 缺 `560c9f3` 与 `0aab69a`，从那里开分支会让 Step 9 的文档 PR **静默 revert 掉这两个 commit**（TODOS.md 242 行 + ENGINEERING.md / PanelView / ViewWiringSuite 的假事实订正）。这是本编排里唯一一处会真正丢工作的地方。
...
**今日基线实测（`9c33dac`）** —— 每一步拿它做前后 diff，别拿文档里的历史数字（如 T17 时期的「652 项」）当自校验目标：

| 项 | 实测 |
|---|---|
| `helper` 测试 | `✓ all 1025 checks passed`（exit 0） |
| `gui` 测试 | `✓ all 1598 checks passed`（exit 0） |
...
git checkout -b feat/master-volume-lock-split 9c33dac   # PR ①（Step 1）
```

**Wave 1 的三路并行（step-1 ∥ step-9 ∥ step-10）需要三个 `git worktree`**，否则三个会话在同一棵树的 `main` 上互相踩：

```bash
git worktree add ../Claudio-e   -b docs/master-volume-eng   9c33dac   # Step 9
git worktree add ../Claudio-bundle -b build/dev-bundle      9c33dac   # Step 10
```
