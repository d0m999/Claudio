# Casks/claudi0.rb — 参考模板，非本仓库直接生效的 Homebrew tap。
#
# Homebrew cask 必须活在一个名为 `homebrew-<tap>` 的独立仓库里（`brew install --cask
# <owner>/tap/claudi0` 才认得到它）。本文件只是留仓库内供人审阅的"最后一次生成结果"预览，
# 不是安装源 —— 真正会被 `brew install` 用到的副本活在 <owner>/homebrew-tap 仓库的
# Casks/claudi0.rb（该仓库需自行创建，见 .github/workflows/release.yml 的 update-cask job 注释）。
#
...
cask "claudi0" do
  ...
  name "claudi0"
  desc "Claude Code + Codex 语义化提示音 · 策展声音包"
  ...
  postflight do
    ...
    system_command "/usr/bin/xattr",
                   args: ["-dr", "com.apple.quarantine", "#{appdir}/claudi0.app"],
                   must_succeed: false
  end
  ...
  caveats do
    <<~EOS
      ...
      系统设置 > 隐私与安全性 > 下滑找到被拦的 claudi0 > 点"仍要打开"。

      打开 claudi0，点菜单栏面板里的「接管 Claude Code」即可 —— 它会装好放声音的小助手、
      复制内置声音包、并追加（不覆盖、自动备份）Claude Code 的 hook 配置。
      也可以在 Terminal 里跑 #{appdir}/claudi0.app/Contents/Resources/bin/claudi0 setup，
