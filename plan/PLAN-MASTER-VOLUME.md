...
**§9 真机走查补记（2026-07-14，D25 兜底条款执行）**：§5.2 的 15 条走查清单已逐条实测，执行者为
Claude（自动化可覆盖的部分）+ 用户（必须真人手/耳的部分），分工原则见下。

**方法学**：`scripts/dev-bundle.sh` 本身有个真 bug（末行 `echo "✅ $APP（..."` 里 `$APP` 后面
直接跟全角括号，当前 locale 下被误判成变量名的一部分，`set -u` 报 `unbound variable`，脚本以退出码 1
收场——但产物其实是对的），已改成 `${APP}` 显式定界修复，不影响本节结论。真机 GUI 自动化踩了两个坑，
记录以防后来者重踩：① **`CGEventPost` 在本环境完全不生效**（两个独立目标——滑块拖拽、包卡点击——
零效果，且面板全程稳定可见，排除了时序/坐标问题），改用 `System Events` 的 `click at {x,y}`（坐标点击）
与 `perform action "AXIncrement"/"AXDecrement"`（语义动作，即 VoiceOver 上下箭头调用的同一条路径）
驱动，两者都验证有效且落盘；② **`NSPopover` 开启动画未播完就操作会全部落空**——`click` 与后续动作之间
必须留 ≥0.3–0.5s，否则截图/坐标定位到的是动画中间帧。**真正的像素级鼠标拖拽手势**（区别于「让数值改变」）
在本环境没有找到可行的合成路径，D9/D20（①）、D45 的连续分数快照（③ 一部分）、D8（⑮）这几条
**必须由人手完成**——已请用户在真机上补测，结果并入下表。

**结果（对照 §5.2:445-461 表格逐条）**：14/15 通过，1 条明确失败。

- ①③④⑤⑥⑦⑨⑩⑫⑬⑭⑮ 全部通过，含两处值得记录的证据强度差异：③ 的 D45 精度断言拿到两个独立真实拖拽样本
  （`0.55`、`0.25`），其中 `0.55` 在二进制浮点里不可精确表示、若无 `snap()` 会露出脏尾数，是比 `0.25`
  更硬的证据；⑧（D12）拿到了诚实报错的真实证据（`chmod 500` 后红色✗+可执行修复指令），但那条路径是
  「打开面板时结构性探测目录可写性」，不是决议原文描述的「拖一下松手瞬间跳回旧值」那个具体动作——
  该状态下滑块和事件行整个不渲染，验不了那个更具体的子断言，精神符合 D12 但形状有出入，如实记录。
- ⑫（D23①）主断言（空 config → 空态 → 点包卡 → 真实包名重建）通过；「静音钮不再造 `selected_pack: ""`」
  那个附带验证坐标一直点不准，**未能验证**（非核心判据，未构成失败）。
- **⑪（D17+D44）❌ 真机验证失败**：系统「文字大小」拉到最高档（`FontSizeCategory global=AX5`）、
  完全重启 app 后复测，主音量行**没有任何变化**——不折行、面板不加宽，与最低档逐像素一致。
  按 Acceptance 要求本轮未动 Swift 代码，只如实记录现象并登记到 TODOS.md
  「主音量行的 Dynamic Type 三级布局在真机上完全不生效」（P1，附带了一个可核验的假设：
  `PanelView.swift` 的 `typeSizeTier` 读的 `@Environment(\.dynamicTypeSize)` 在 macOS 上
  可能压根不跟随这个系统偏好，与另一条既有 TODO「`DynamicTypeSize → PanelTypeSizeTier` 映射用裸
  `default:`」共享同一处代码、可能共用一次修复窗口）。

**结论**：D25 的兜底条款（⑨ 由人眼守门）与其余 14 条一并执行完毕，**但 §9 不是全绿**——阶段 D 交付
声明里「Dynamic Type 三级布局」这一项在真机上不成立，需求方需要决定是在 TODOS 里挂账继续，
还是把它拉回本方案重新开一轮决议。**其余 14 条的验收结论确定：可以收尾。**
