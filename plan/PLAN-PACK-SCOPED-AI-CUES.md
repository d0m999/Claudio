...
可选用。声音页默认选中第一个事件；点击事件名称、开始生成或从缺声深链进入时，在该事件行
下方固定显示 Claude Code、Codex、WorkBuddy 三行来源对照，切换事件时更新同一处对照，不再
另设能力查看动作。主视图使用「支持」「仅授权请求可用」「暂不可用，尚未接入」和
「不可用，这个来源没有此类事件」；原生事件名与精确能力状态放在默认收起的「技术信息」中。
对照仍从宿主 adapter 的 `HostCapabilityBinding` 投影，明确区分来源对事件的支持、实现与当前
激活；它只说明事件能力，不承诺当前一定播放。实际能否听到声音还取决于是否选用此包、连接、
静音及该事件是否已有音频。能力对照不限制包级生成，缺声入口保留在「事件与提示音」并进入
上述深链。
...
- 原型走查覆盖五个事件、默认选中与事件切换、Codex「执行中断」和「待响应」、WorkBuddy
  未接入事件的文案、默认收起且可用键盘展开的「技术信息」、生成按钮不受来源状态禁用、
  缺声深链，以及共用与继承、用户包原地采纳、Global/Surface 内置包深链、任意包复制、
